...
剩下的是 6 項**已記錄的取捨**（不是缺陷）與一項刻意未動的 P3 功能（行程地圖，需合規圖資）。
...
### 4.1 六項已記錄的取捨（技術債 #3–#8）

這些**不是缺陷**，是單副本部署下的合理選擇；多副本上線前必須處理：

| # | 問題 | 影響 | 處理方向 |
|---|------|------|----------|
| 3 | Redis 不可用時限流退回**記憶體**（fail-open） | 多副本下各副本各自計數，限流不再共享 | 監控告警 Redis 健康；必要時改 fail-closed |
...
| 6 | WebSocket 狀態存**單程序記憶體** | 多副本下廣播不跨程序 | 改 Redis Pub/Sub |
| 7 | 前端無狀態管理庫（只有 React Context） | 規模變大後快取／重試會變手工 | 引入 TanStack Query |
| 8 | 雜湊線程池是**每程序**上限 | 總並發 = 副本數 × `PASSWORD_HASH_MAX_CONCURRENCY` | 依記憶體規劃調整 |
...
- **P3：行程地圖**。規格明訂「若加入地圖，**必須**遵守地圖合規（使用合規圖資）」，
  這需要先決定圖資供應商與合規方案，不應由實作端自行決定。
